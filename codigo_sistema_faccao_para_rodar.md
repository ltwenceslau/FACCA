...
function normalizarCadastro(lista,item){
  ...
  if(lista==="modelos"){item.valorColaborador=Number(item.valorColaborador||0);item.valorTerceiro=Number(item.valorTerceiro||0);item.descricao=item.descricao||item.obs||"";item.aviamentos=item.aviamentos||""}
